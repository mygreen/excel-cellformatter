...
public class ExcelDateUtils {
     ...
     *         ただし、この値はタイムゾーンは考慮されていない（=GMT-00:00）であるため、
     *         独自に処理を行う必要がある。
     */
    public static Date convertJavaDate(final double numValue, final boolean startDate1904) {
        ...
        if(startDate1904) {
             ...
             * ・しかし、グレゴリオ歴上は1900年は閏年ではないため、UTCの表現上1日多い。
     ...
     * <p>少数は第13位まで保証し、小数第14位は四捨五入して計算します。
     *    Excelの1秒は、UTC上では1/(60x60x24x1000)=0.0000000115741=1.15741e-008であるので、小数13位まで保証すればよいため。
     * <p>さらに、UTC上は1900年2月29日は存在しないため、表現できません。
     * 
    ...
    public static double convertExcelNumber(final Date value, final boolean startDate1904) {
        ...
        numValue = numValue.divide(new BigDecimal(SECONDS_IN_DAYS * 1000), 13, BigDecimal.ROUND_HALF_UP);
     ...
     * <p>1900年始まりの場合、Excelでは1月0日から始まるため、{@literal 1899-01-01 00:00:00.000}の値を返す。
